...
dimer.set_pbc([0, 0, 0])
calc = NequIPCalculator.from_deployed_model('../adpmodel40ku.pth', device='cuda')
dimer.set_calculator(calc)

for i in range(80000):
    atoms = read('../get-pes-small/test80.extxyz',index=i)
    ...
    if index_x >= 0 and index_x < binsx:
        if index_y >= 0 and index_y < binsy:
...
np.savetxt('enthalpy.txt', np.column_stack([grid, hist.flatten()]))
np.savetxt('hist.txt', np.column_stack([grid, numbers.flatten()]))
